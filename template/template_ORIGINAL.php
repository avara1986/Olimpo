...
class Template
{
	 ...
	 * @access public
	 */
	public function __construct($template_path='template', $cache_path='cache', &$lang=NULL)
	{
		if (file_exists($template_path))
		{
			$this->root = $template_path;
			$this->cachepath = $cache_path . '/tpl_';
			$this->lang = &$lang;
		}
		else
		trigger_error('Template path could not be found: ' . $template_path, E_USER_ERROR);
	 ...
	 * @access public
	 */
	function set_custom_template($template_path, $cache_path)
	{
		$this->root = $template_path;
		$this->cachepath = $cache_path . '/tpl_';
		return true;
	 ...
	 * @access public
	 */
	function set_language_var(&$lang)
	{
		$this->lang = &$lang;
	}
	/**
	 ...
	 * @access private
	 */
	private function _tpl_load_file($handle, $store_in_db = false)
	{
		...
		$this->compiled_code[$handle] = $this->compile(trim(@file_get_contents($this->files[$handle])));
...
'#<\?php(?:\r\n?|[ \n\t]).*?\?>#s'
	 ...
	 * @access private
	 */
	private function compile($code, $no_echo = false, $echo_var = '')
	{
		if ($echo_var)
		global $$echo_var;
	...
	private function compile_var_tags(&$text_blocks)
	{
		...
		{
			$text_blocks = preg_replace('#\{L_([a-z0-9\-_]*)\}#is', "<?php echo ((isset(\$this->_rootref['L_\\1'])) ? \$this->_rootref['L_\\1'] : ((isset(\$this->lang['\\1'])) ? \$this->lang['\\1'] : '{ \\1 }')); ?>", $text_blocks);
		}
		// Handle addslashed language variables prefixed with LA_
		// If a template variable already exist, it will be used in favor of it...
		if (strpos($text_blocks, '{LA_') !== false)
		{
			$text_blocks = preg_replace('#\{LA_([a-z0-9\-_]*)\}#is', "<?php echo ((isset(\$this->_rootref['LA_\\1'])) ? \$this->_rootref['LA_\\1'] : ((isset(\$this->_rootref['L_\\1'])) ? addslashes(\$this->_rootref['L_\\1']) : ((isset(\$this->lang['\\1'])) ? addslashes(\$this->lang['\\1']) : '{ \\1 }'))); ?>", $text_blocks);
		}
